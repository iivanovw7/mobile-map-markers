 ...
 * @param {number} min - min-width breakpoint key.
 * @param {number} max - max-width breakpoint key.
 *
 ...
 * }`
 *
 * `@mixin media width, zero, sm {
 *  font-size: units(35);
 * }`
 */
function media(mixin, type, min, max ) {
    const breakPointsSet = type === 'width'
        ? breakpoints
        : screenHeight;

    const minQuery = `@media only screen and (min-${type}: ${breakPointsSet[min]}px)`;
    const mediaQuery = max
        ? (`${minQuery} and (max-${type}: ${breakPointsSet[max]}px)`)
        : minQuery;
